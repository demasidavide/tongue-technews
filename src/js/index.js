...
export function createCard(cardData, classStyle, position, options = {}) {
  ...
  const imgTopScore = showTop
    ...
    : "";
  const heartFill = heart ? `active` : `heart`;
  ...
  headerTop.addEventListener("click", (e) => {
    ...
    if (e.target.closest("#heartIcon")) {
      if (saveFun) {
        ...
        let favoritesArray =
          JSON.parse(localStorage.getItem("favorites")) || []; // Recupera preferiti o array vuoto
    ...
    .then((data) => {
      ...
      validNews.forEach((element) => {
        ...
        fetch(apiBase + `item/${element}.json`)
          ...
          .catch((error) =>
            console.error("Errore,impossibile creare la Card", error)
          );
      });
    })
    .catch((error) => {
